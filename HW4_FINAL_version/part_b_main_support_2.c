...
void print_player(void *item){
  //this will print PLAYER's name and color out
  //there is a SQUARE in PLAYER, as ptr_to_node
  PLAYER local_player=(PLAYER)item;
  //local now have my PLAYER info
  // char *name = malloc(sizeof(char)*20);
  //
  // strcpy(name,local_player->name);
  // printf("%s\n",name );
  char*color_in_s=get_color(local_player->ptr_to_node);//this is a SQUARE
  printf("%s\n",color_in_s );
